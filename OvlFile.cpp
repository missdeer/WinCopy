...
namespace io
{
    ...
    OvlFile::OvlFile(const io::IoPool &ioPool, const wchar_t *const fileName, const DWORD disposition, const DWORD access, const DWORD shareMode)
    {
        if (disposition == CREATE_ALWAYS)
        {
            if (PathFileExistsW(fileName))
            {
                ...
                {
                    std::wstring dst(fileName);
                    dst.append(L"~");
                    MoveFileExW(fileName, dst.c_str(), MOVEFILE_DELAY_UNTIL_REBOOT | MOVEFILE_REPLACE_EXISTING);
                }
            }
        }
        // create file
        HANDLE file = ::CreateFileW(fileName, access, shareMode, nullptr, disposition, FILE_FLAG_OVERLAPPED, nullptr);
    ...
    void OvlFile::StartRead(uint64_t fileOffset, char *data, DWORD size, CompleteHandler_t handler)
    {
        ...
        const BOOL result = ::ReadFile(m_file.get(), data, size, nullptr, &fileOverlapped->m_ovlPlus.m_ovl);
    ...
    void OvlFile::StartWrite(uint64_t fileOffset, const char *data, DWORD size, CompleteHandler_t handler)
    {
        ...
        const BOOL result = ::WriteFile(m_file.get(), data, size, nullptr, &fileOverlapped->m_ovlPlus.m_ovl);
    ...
    std::unique_ptr<OvlFile::FileIoOverlapped> OvlFile::PrepareOverlapped(uint64_t fileOffset, CompleteHandler_t handler)
    {
        std::unique_ptr<FileIoOverlapped> fileOverlapped = std::make_unique<FileIoOverlapped>();
