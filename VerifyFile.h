...
namespace verify
{
    ...
    class VerifyFile : public std::enable_shared_from_this<VerifyFile>
    {
    public:
        using CompleteHandler_t = std::function<void()>;

        VerifyFile(const io::IoPool &ioPool, const std::wstring &srcPath, const std::wstring &dstPath, CompleteHandler_t onComplete)
    ...
    private:
        void StartRead()
        {
            io::CompleteHandler_t completeHandler = std::bind(std::mem_fn(&VerifyFile::OnReadComplete), shared_from_this(), std::placeholders::_1);
        ...
        void OnReadComplete(DWORD transferred)
        {
            try
            {
                ...
                io::CompleteHandler_t completeHandler =
                    std::bind(std::mem_fn(&VerifyFile::OnWriteComplete), shared_from_this(), std::placeholders::_1);
