...
namespace digest
{
    ...
    class DigestFile : public std::enable_shared_from_this<DigestFile>
    {
    public:
        using CompleteHandler_t = std::function<void()>;

        DigestFile(const io::IoPool &ioPool, const std::wstring &srcPath, const std::wstring &dstPath, CompleteHandler_t onComplete)
    ...
    private:
        void StartRead()
        {
            io::CompleteHandler_t completeHandler = std::bind(std::mem_fn(&DigestFile::OnReadComplete), shared_from_this(), std::placeholders::_1);
        ...
        void OnReadComplete(DWORD transferred)
        {
            try
            {
                ...
                io::CompleteHandler_t completeHandler =
                    std::bind(std::mem_fn(&DigestFile::OnWriteComplete), shared_from_this(), std::placeholders::_1);
