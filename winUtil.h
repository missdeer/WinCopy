...
namespace utils
{
    ...
    public:
        State(const std::wstring &mask = L"", const std::wstring &path = L"") : m_handle(0), m_path(path), m_mask(mask), m_context(0)
        {
            m_query = path + mask;
        }
    };
    ...
    {
        SM_SEARCH_FILE   = 0x1,
        SM_SEARCH_FOLDER = 0x2,
        SM_SEARCH_ALL    = 0xF
    } TSearchMode;

    class CSearcher
    {
        template<class ContainerType> class CCleaner
        {
        ...
        public:
            CCleaner(ContainerType *pContainer, CSearcher *pSearcher) : m_pContainer(pContainer), m_pSearcher(pSearcher) {}
            ~CCleaner()
            {
                for (auto f : *m_pContainer)
                {
                    m_pSearcher->FindClose(f);
                }
            }
        };

        using CStateList = std::list<State>;
        CStateList        m_states;
    ...
    public:
        ...
        void StartSearch(const std::wstring &wsMask, const std::wstring &wstrPathName, int iMaximumLevels);
